...
describe("Queens block counters against their owner's one-offs", () => {
  it("responder with a 2 cannot counter when the one-off player has a Queen", () => {
    const s = aceSetup([], [card(2, C)]);
    s.royals.p1 = [{ c: card(Q, H), g: false }];
    actOneOff(s, "p1", card(A, C), null);
    // no counter window at all — resolved immediately
    expect(s.phase).toBe("play");
    expect(s.points.p2).toHaveLength(0);
    expect(s.hands.p2).toEqual([card(2, C)]);
  });

  it("a Glasses eight is not a Queen and blocks nothing", () => {
    const s = aceSetup([], [card(2, C)]);
  ...
  it("queen protection applies per link: countering player's Queen blocks the re-counter", () => {
    // p2 has a queen; p1 plays one-off; p2 counters; p1 holds a 2 but cannot
    // counter p2's 2 because p2's queen protects it in suspension.
    const s = aceSetup([card(2, D)], [card(2, C)]);
    s.royals.p2 = [{ c: card(Q, S), g: false }];
    actOneOff(s, "p1", card(A, C), null);
    expect(s.phase).toBe("counter");
    playCounterTwo(s, "p2", card(2, C));
    // p1's re-counter window is denied → cancelled immediately
    expect(s.phase).toBe("play");
    expect(s.points.p2).toHaveLength(1);
    expect(s.hands.p1).toContain(card(2, D)); // never spent
  });
});
