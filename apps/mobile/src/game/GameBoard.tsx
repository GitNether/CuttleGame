...
export interface GameBoardProps {
  state: GameState;
  me: PlayerId;
  /** Bumps whenever the authoritative state advances — used to clear the local
   *  selection so a stale pick can't be submitted. */
  stateKey: number;
  busy: boolean;
  commit: (mutator: (g: GameState) => boolean) => void;
  onLeave: () => void;
  /** Room code (online) or undefined for the offline "vs Computer" game. */
  roomCode?: string;
  /** Offline: the local player drives both seats, so the rematch is always
   *  available to them regardless of who won. */
  localControlsBothSeats?: boolean;
}
...
export function GameBoard({
  ...
  // Any advance of the authoritative state invalidates in-flight selections.
  useEffect(() => {
    setSelCard(null);
    setTargetMode(null);
    setDiscardSel([]);
  ...
  if (s.phase === "over")
    banner =
      s.winner === "draw"
        ...
        : `Game over — ${s.names[s.winner as PlayerId]} wins! 🏆`;
    ...
    return (
      <View style={styles.actions}>
        ...
        <View style={styles.arow}>
          {opts}
          {jackBlocked && <Hint>Jack is blocked — {s.names[opp]} has a Queen.</Hint>}
          {fromSeven && noPlay && (
            <Btn
              kind="warn"
  ...
  const Field = ({ p }: { p: PlayerId }) => (
    <>
      ...
      <RowLabel>
        Point cards{" "}
        {p === me ? `(you: ${pointsOf(s, p)}/${goalOf(s, p)})` : `(${pointsOf(s, p)}/${goalOf(s, p)})`}
      </RowLabel>
      <View style={styles.cardrow}>
  ...
  const rematchStarter: PlayerId = s.winner === "draw" ? "p1" : (s.winner as PlayerId);
  const canRematch =
    s.phase === "over" &&
    (localControlsBothSeats || (s.winner === "draw" ? me === "p1" : s.winner === me));
      ...
      <ScrollView contentContainerStyle={styles.wrap}>
        ...
        <View style={styles.topbar}>
          <Text style={styles.title}>
            🦑 Cuttle <Text style={{ color: colors.gold }}>{roomCode ? "Online" : "Solo"}</Text>
          </Text>
            ...
            <View style={styles.pill}>
              {roomCode ? (
                <Text style={{ color: colors.text, fontSize: 13 }}>
                  Room{" "}
                  <Text style={{ color: colors.gold, letterSpacing: 2, fontWeight: "700" }}>{roomCode}</Text>
                </Text>
              ) : (
                <Text style={{ color: colors.text, fontSize: 13 }}>vs Computer 🤖</Text>
              )}
            </View>
        ...
        {waitingForP2 && roomCode && (
          <Notice>
            Share the room code {roomCode} with your friend. They enter their name and the code and
          ...
          <View style={styles.gameover}>
            ...
            {canRematch ? (
              <Btn
                ...
                title="Rematch"
                disabled={busy}
                onPress={() => commit((g) => actRematch(g, rematchStarter))}
              />
            ) : (
              <Hint>
                Waiting for {s.names[rematchStarter]} to start the rematch…
              </Hint>
            )}
          </View>
        )}
          ...
          <View style={styles.cardrow}>
            {s.hands[me].map((c, i) => (
              <CardView
                ...
                id={c}
                sel={selCard === c}
                onPress={myTurn && !targetMode ? () => setSelCard(selCard === c ? null : c) : undefined}
              />
            ))}
